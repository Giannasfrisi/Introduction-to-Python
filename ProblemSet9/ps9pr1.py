...
class Board:
    ...
    def reset(self):
        ...
        called by setting all slots to contain 
        a space character."""
        for h in range(self.height):
    ...
    def can_add_to(self, col):
        """returns True if it is valid to place 
    ...
    def remove_checker(self, col):
          ...
          col of the called Board object"""
          for h in range(self.height):
              if self.slots[h][col] != ' ':
                  self.slots[h][col] = ' '
                  break
    ...
    def is_win_for(self, checker):
        """accepts a parameter checker that is either 
        'X' or 'O', and returns True if there are four consecutive 
        slots containing checker on the board. Otherwise, it should return False."""
    ...
    def is_down_diagonal_win(self, checker):
        ...
        for row in range(self.height):
            for col in range(self.width - 3):
                 if row - 3 >= 0 and col + 3 <= self.width:
                    if self.slots[row][col] == checker and \
